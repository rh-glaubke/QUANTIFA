...
% Tropical Pacific Ocean Variant
%
% ---- About ----
%
% QUANTIFA is a user-friendly exploratory and data analysis tool for
% analyzing individual foraminiferal data, constraining the uncertainties
% inherent to IFA statisics, and testing the resolving power of IFA
% populations in detecting changes in annual and interannual climate 
% variability. Please see the README document for a quickstart guide on
% how to use the algorithm. For a more detailed description of the algorithm's
% statistical framework, please see the associated publication cited below.
% 
...
% version of the script is designed to work with data from the tropical
% Pacific (TP_ORAS5.mat), which can be downloaded in the QUANTIFA
% repository on GitHub (https://github.com/rh-glaubke/QUANTIFA). For any 
% information regarding the ORA-S5 dataset, please refer to Zuo et al. 
% [2019] (doi:10.5194/os-15-779-2019) or visit the Ocean Synthesis/
% Reanalysis Directory of the Integrated Climate Data Center: 
...
% 
% Citation: Glaubke et al. (submitted). Discerning changes in high-frequency
%           climate variability using geochemcial populations of individual 
%           foraminifera. Submitted to Paleoceanography and Paleoclimatology.
...
%
%     NOTE: if you would like to run an exploratory sensitivity analysis,
...
%
% Reanalysis Dataset:
%     TP_ORAS5.mat is a workspace file containing an ORA-S5 data structure 
%     w/ a gridded field of potential temperature data ('votemper'; 1°x1° 
%     horizontal resolution) for 75 analyzed depth levels ('depths'). It is 
%     available for download here: https://github.com/rh-glaubke/QUANTIFA
%
% Location of Core:
%     lat - Latitude for temperature gridpoint (Format: XY.5)
%     lon - Longitude for temperature gridpoint (Format: XY)
%     dep - Mean habitat depth for foram species of interest (Format: XY)
%
...
%
% Input Parameters:
%     A set of input conditions used to initialize the Monte Carlo picking
%     algorithm and parameterize the modeled pseudoproxy time series.
%     Breif description of each term is below.
%
% Output Products:
%     QUANTIFA produces the following figures:
%     1) A contour plot showing the relative influence of seasonality and
%        ENSO on the tails and body of the paleotemperature distribution.
%     2) A Q-Q plot of the user's data (or the user's data vs. ORA-S5 data)
%     3) Heat maps illustrating the proportion of signficiant quantiles
%        that align with each altered paleoclimate scenario, where the 
%        darker colors indicate scenarios that are most consistent with the
%        user's data.
%     
%     There is also a suite of supplemental figures, including:
...
load('TP_ORAS5.mat');           % Call up ORA-S5 dataset 
                                % (download link for .mat file above)

% ---- Location of Core ----

lat = -0.5;                     % Data point for MV1014-02-17JC (EEP)
lon = -85;
dep = 40;                       % NOTE: QUANTIFA selects the ORA-S5 depth
...
eqn = 5;                        % 1 = Anand et al. (2003) All species eqn
...
seas = [3 4 5];                 % Seasonal weight for picking algorithm (eg
                                % [1:1:12] for all-year; [12 1 2] for DJF)
tsl = 120;                      % Length of synthetic time series (yrs)
cl = 90;                        % Confidence level (80, 90, 95, 99)
anerr = 0.055;                  % Analytical error (mmol/mol)
calerr = 1.6;                   % Calibration error (°C) [OPTIONAL]
...
disp('(QUANTIFA) - Tropical Pacific Variant');
disp(' ');
disp('Written by Ryan Glaubke and Kaustubh Thirumalai'); 
disp('Last Updated: July 2020');
disp('Glaubke et al. (submitted), Paleoceanography & Paleoclimatology');
disp('doi:XXXX');
disp('------------------------------------------------------------------------');
...
if rem(lat,1) == 0                  % Check proper formatting: latitude
    disp(' ');
    error('Error: Latitude is a whole number. Please input latitude as XY.5');
end
...
if lon < 0                          % Check model domain: longitude
    if (360+lon > 290) || (360+lon < 120)
        disp(' ');
        error('Error: Entered longitude is out of model domain [120°E - 70°W].');
    end
else
    if (lon > 290) || (lon < 120)
        disp(' ');
...
if eqn == 3 || eqn == 5               % Check for dissolution correction term    
    if exist('D','var') == 0
        error('Error: no depth correction term found! Please create a variable, D, and assign it a value equal to the depth of your core (in kilometers).');
    end
end
...
for i=1:len
    T(i) = ORAS5.votemper(ilon,ilat,idepth,i);
end
...
switch eqn
    ...
    case 5  % Dekens et al. (2002) N. dutertrei eqn (w/ dissolution corr.)
        for i = 1:len
...
%% Climatology
% Extracts seasonal cycle, El Niño and La Niña climatology (beginning with 
% the month of April) from forward-modeled Mg/Ca time series.
% 
% If your time series is longer than 1958-present, you have to manually 
...
elyear = [1963;1965;1968;1972;1982;1986;1987;1991;1994;1997;2002;2009;2015];
layear = [1970;1971;1973;1975;1984;1988;1995;1998;1999;2000;2007;2010;2011];
...
for month=1:12
    Cel(month) = nanmean(elt(month:12:end));
    Cla(month) = nanmean(lats(month:12:end));
...
% ---- ENSO Deviations from Base Climatology ----
...
if (nanmean(Cel) < nanmean(Cla))        % eg. in SW Pacific, ENSO = cold
    wp = 0;                             % and dry
else
    wp = 1;
end
...
% Constructs synthetic time series mimicking 'normal' modern variability. 
% Red and white noise is added and altered according to a while loop check 
...
while ((mdif > aitol) || (wnp > wth) || (rnp > rth))
    ...
    % ---- Insert El Nino Events ----

    for i = 1:length(elin)
        if (wp == 0)
            mod_clim(:,elin(i)) = Cel - rand*Celsd*mag;
        else
            mod_clim(:,elin(i)) = Cel + rand*Celsd*mag;
        end
    end
    
    % ---- Insert La Nina Events ----
    
    for i = 1:length(lain)
        if (wp == 0)
            mod_clim(:,lain(i)) = Cla + rand*Clasd*mag;
        else
            mod_clim(:,lain(i)) = Cla - rand*Clasd*mag;
        end
    end
...
% Constructs 441 synthetic time series with altered ENSO/Seasonal Cycle
% amplitudes. ENSO/Seasonal Cycle amplitudes vary from 100% dampened (or no
% signal) to 100% amplified (or double signal). 21 hypothetical seasonal
% cycle scenarios x 21 hypothetical ENSO scenarios = 441 total altered time 
% series.
...
for p = -100:10:100                         % Seasonal Cycle Amplitude
    c = 1;
    for w = -100:10:100                     % ENSO Amplitude
        ...
        for i = 1:length(elin)
            if (wp == 0)
                alt_clim(:,elin(i)) = nCel - rand*Celsd*mag;
            else
                alt_clim(:,elin(i)) = nCel + rand*Celsd*mag;
            end
        end
        ...
        for i = 1:length(lain)
            if (wp == 0)
               alt_clim(:,lain(i)) = nCla + rand*Clasd*mag;
            else
               alt_clim(:,lain(i)) = nCla - rand*Clasd*mag;
            end
        end
...
% Computes normalized quantiles from the modeled modern time series and
% from each of the altered time series. Also computes the residual (or 
% distance from 1:1 line) for each individual quantile in each scenario.
...
%% Testing Paleotemperature Distribution Sensitivity
...
%% Output Product #1: Paleotemperature Distribution Sensitivity
...
for i = 1:3
    ...
    ylim([-100 100]);
    caxis([0 100]);
    xlabel('Change in ENSO Amplitude (%)','FontName','Myriad Pro','fontsize',14,'Fontweight','demi');
    ylabel('Change in Seasonal Cycle (%)','FontName','Myriad Pro','fontsize',14,'Fontweight','demi');
    set(gca,'LineWidth',1.5,'fontsize',14,'Fontweight','demi');
    set(gca,'Xtick',-100:20:100)
    set(gca,'XtickLabel',[-100:20:100])
    
end
% set(gcf,'position',[100,50,2000,700]);
f1.GraphicsSmoothing = 'on';

% -- Save as PDF --

if savefig == 1
    f_filename = 'INFAUNALv2_SensitivityPlot';
...
% Estimates total uncertainty for inputted IFA data in X and Y via a Monte 
% Carlo approach. Uncertainty is estimated by repeatedly resampling the
% modeled modern time series. Estimation takes analytical and sampling
% uncertainty into account, as well as the error on the Mg/Ca-T 
% calibration.
...
% scenario (i.e. ∆R). ∆R is then used to identify quantiles which deviate
% from the 1:1 line.
...
%% Output Product #3: "Goodness-of-Fit" Heat Maps
...
idx = find(lval>=j);
% idx(idx > max(ct) & idx < min(wt)) = []; % Only look at sig quantiles in tails
                                         % (added 09/30/2020 for paleo-ENSO
                                         % manuscript. - RG)
if isempty(idx)
    if j == 1
...
if savefig == 1
    f_filename = 'INFAUNALv2_deltaR_heatmaps';
    export_fig(f_filename, '-pdf', '-append', '-transparent', '-r600');
end
...
clearvars -except alt_qs alt_series anerr bdyp bins calerr cl ctp D d_diff ...
    dep depths edges envX envY eqn fx fy lat lon lval M m_qx m_qy MgCa ...
    mod_qs mod_series nrst_depth num_p num_q numX numY ORAS5 pfit qx qy ...
    serrX serrY savefig T tmslice wtp X xi xr_data Y yi yr_alt yr_data
...
%% Functions
function [ilat,ilon,idepth] = oraco(lat,lon,nrst_depth,depths)
% Converts lat/lon/dep input into corresponding ORA-S5 matrix indicies
%
% - created by R. Glaubke (10/21/2019) after 'hadco' function by K. Thiurmalai
